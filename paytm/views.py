...
from paytm.models import PaytmDataBase  # ,PaytmRefundDataBase
        ...
        if self.request.POST.get('PAYMENT_MODE_ONLY') == 'Yes':
            if not all(key in self.request.POST for key in ('AUTH_MODE', 'PAYMENT_TYPE_ID', 'CARD_TYPE', 'BANK_CODE')):
                raise ValidationError(
                    "please provide all these fields too: ('AUTH_MODE', 'PAYMENT_TYPE_ID', 'CARD_TYPE', 'BANK_CODE')")
...
class PaytmResponse(TemplateView):
    ...
    def save_transection(self, response_data):
        PaytmDataBase.objects.update_or_create(order_id=response_data.get('ORDERID'),
                                               amount=response_data.get(
                                                   'TXNAMOUNT'),
                                               checksumhash=response_data.get(
                                                   'CHECKSUMHASH'),
    ...
    def get_context_data(self, **kwargs):
        ...
        verified = verify_checksum(
            response_data, settings.PAYTM_MERCHANT_KEY, response_data['CHECKSUMHASH'])
...
def paytm_transection_status(request):
    if request.method == 'POST':
        order = get_object_or_404(
            PaytmDataBase, order_id=request.POST.get('ORDER_ID'))
        data = {}
        data.update({
            'MID': paytm_sets['MID'],
